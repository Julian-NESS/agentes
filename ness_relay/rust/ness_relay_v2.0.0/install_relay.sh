...
# Redirige llamadas de nombres viejos a las funciones nuevas
menu_switches() { menu_switches_tester "$@"; }
menu_firewalls() { menu_firewalls_tester "$@"; }
menu_routers() { menu_routers_tester "$@"; }
menu_wireless() { menu_wireless_tester "$@"; }

# ===============================================================================
# --- FUNCIONES DE MENÚ TESTER (VERSIÓN FINAL DEFINITIVA) ---
# ===============================================================================

menu_switches_tester() {
    echo -e "\n${WHITE}${BOLD}CATEGORÍA: SWITCHES${NC}"
    echo -e "  1) Cisco\n  2) Huawei\n  3) Dell\n  4) Datacomm\n  5) TP-Link\n  v) Volver atrás"
    read -p "Selecciona marca [1-5 o v]: " sw_tester_choice
    case "$sw_tester_choice" in
        1) AC_VENDOR="cisco" ;; 2) AC_VENDOR="huawei" ;; 3) AC_VENDOR="dell" ;;
        4) AC_VENDOR="datacomm" ;; 5) AC_VENDOR="tp_link" ;; "v"|"V") return 1 ;;
        *) AC_VENDOR="generic" ;;
    esac
    vendor=$AC_VENDOR
    # --- ESTO ARREGLA EL RESUMEN ---
    SELECTED_VENDORS["$vendor"]="true"
    DEVICE_CONFIGS["${vendor}_count"]=1
}

menu_routers_tester() {
    echo -e "\n${WHITE}${BOLD}CATEGORÍA: ROUTERS${NC}"
    echo -e "  1) MikroTik\n  v) Volver atrás"
    read -p "Selecciona marca [1 o v]: " rt_tester_choice
    case "$rt_tester_choice" in
        1) AC_VENDOR="mikrotik" ;; "v"|"V") return 1 ;; *) AC_VENDOR="generic" ;;
    esac
    vendor=$AC_VENDOR
    # --- ESTO ARREGLA EL RESUMEN ---
    SELECTED_VENDORS["$vendor"]="true"
...
menu_firewalls_tester() {
    echo -e "\n${WHITE}${BOLD}CATEGORÍA: FIREWALLS${NC}"
    echo -e "  1) Fortinet\n  2) pfSense\n  3) Sophos\n  4) MikroTik FW\n  v) Volver atrás"
    read -p "Selecciona marca [1-4 o v]: " fw_tester_choice
    case "$fw_tester_choice" in
        1) AC_VENDOR="fortinet" ;; 2) AC_VENDOR="pfsense" ;; 3) AC_VENDOR="sophos" ;;
        4) AC_VENDOR="mikrotik_fw" ;; "v"|"V") return 1 ;; *) AC_VENDOR="generic" ;;
    esac
    vendor=$AC_VENDOR
    # --- ESTO ARREGLA EL RESUMEN ---
    SELECTED_VENDORS["$vendor"]="true"
...
menu_wireless_tester() {
    echo -e "\n${WHITE}${BOLD}CATEGORÍA: WIRELESS${NC}"
    echo -e "  1) Ubiquiti (UBNT)\n  v) Volver atrás"
    read -p "Selecciona marca [1 o v]: " wl_tester_choice
    case "$wl_tester_choice" in
        1) AC_VENDOR="ubnt" ;; "v"|"V") return 1 ;; *) AC_VENDOR="generic" ;;
    esac
    vendor=$AC_VENDOR
    # --- ESTO ARREGLA EL RESUMEN ---
    SELECTED_VENDORS["$vendor"]="true"
...
# Definir fabricantes disponibles (ACTUALIZADO 2026)
VENDORS=("cisco" "huawei" "dell" "datacomm" "tp_link" "mikrotik" "fortinet" "pfsense" "sophos" "mikrotik_fw" "ubnt")

VENDOR_NAMES=("Cisco Switches" "Huawei Switches" "Dell Switches" "Datacomm" "TP-Link" "MikroTik Router" "Fortinet FW" "pfSense FW" "Sophos XGS" "MikroTik FW" "Ubiquiti AP")

VISIBLE_VENDOR_COUNT=${#VENDOR_NAMES[@]}
...
if [[ "$SILENT_MODE" != "true" && -f "$AUTOCOMPLETE_FILE" ]]; then
    ...
    if [[ "$USE_AUTOCOMPLETE" =~ ^[Yy]$ ]] || [[ -z "$USE_AUTOCOMPLETE" ]]; then
        ...
        # --- NUEVO: Selección de marca para el Autocompletado con Retorno ---
while true; do
    echo ""
    print_box "CATEGORÍA DEL DISPOSITIVO" "${CYAN}${BOLD}"
    echo -e "${WHITE}Selecciona la categoría para el dispositivo validado:${NC}"
    echo -e "  1) Switches"
    echo -e "  2) Routers"
    echo -e "  3) Firewalls"
    echo -e "  4) Wireless"
    echo ""
    echo -ne "${BOLD}Selecciona [1-4]: ${NC}"
    read CAT_CHOICE_AC

    case "$CAT_CHOICE_AC" in
        1) menu_switches_tester && break ;;
        2) menu_routers_tester && break ;;
        3) menu_firewalls_tester && break ;;
        4) menu_wireless_tester && break ;;
        *) echo -e "${RED}⚠️ Opción inválida, intenta de nuevo.${NC}" ; sleep 1 ;;
    esac
    # Explicación: Si en la función presionas 'v', ésta devuelve un error (return 1).
    # Como usamos '&& break', el break NO se ejecuta y el bucle while vuelve a empezar.
done
...
if [[ "$AUTOCOMPLETE_USED" == "true" ]]; then
    ...
    while true; do
        ...
        case "$CAT_CHOICE_AC" in
            ...
            3) menu_firewalls_tester && break ;;
            4) menu_wireless_tester && break ;;
            *) echo -e "${RED}⚠️ Opción inválida${NC}" ; sleep 1 ;;
        esac
    done
else
