...
///
/// **Audit data gate** (Phase 2.4): if the engine produced
/// `vulnerabilities` / `cis_compliance` keys, they are stripped by default.
/// The server-side Django schema has no columns for them yet — sending the
/// raw audit payload would result in a 400 or 500 and the regular SNMP
/// payload would be lost.
///
/// To temporarily forward audit data to the server (e.g. during a
/// coordinated migration with the backend team), set the env var:
/// ```bash
/// NESS_SEND_VULNERABILITIES=true ./ness-relay --audit
/// ```
pub fn transform_for_server(mut payload: Value) -> Value {
    ...
    transform_audit(&mut payload);
    payload
}

/// Audit-data gate. Strips `vulnerabilities` and `cis_compliance` from the
/// payload before it reaches `json_exporter` and `server_sender`.
///
/// The audit blocks **always** end up in the local `relay_data.json` so
/// operators can review them — this function only controls whether they
/// are forwarded to the server.
fn transform_audit(payload: &mut Value) {
    let send_audit = std::env::var("NESS_SEND_VULNERABILITIES")
        .map(|v| v.eq_ignore_ascii_case("true") || v.trim() == "1")
        .unwrap_or(false);
    if !send_audit {
        if let Some(obj) = payload.as_object_mut() {
            obj.remove("vulnerabilities");
            obj.remove("cis_compliance");
        }
        return;
    }
    // When explicitly enabled, leave the keys in place. The backend team
    // is responsible for accepting the new schema or rejecting it.
}
