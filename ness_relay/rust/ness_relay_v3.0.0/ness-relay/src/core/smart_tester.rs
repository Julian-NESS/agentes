...
pub async fn run_verify_setup(
    ...
    // Phase 2.5: Soporte multi-IP manual. Si no hay devices en config,
    // preguntamos UNA o MÁS IPs para validar conectividad SNMP. Esto
    // reemplaza al flujo anterior que solo aceptaba 1 IP.
    let manual_ips: Vec<String> = if devices.is_empty() {
        prompt_manual_target_ips(assume_yes)?
    } else {
        Vec::new()
    };

    let reachable_targets = phase_b_network_health(&devices, &endpoint, &manual_ips).await;

    // Phase 2.5: Validar SNMP para CADA IP manual ingresada.
    if devices.is_empty() {
        for (i, ip) in manual_ips.iter().enumerate() {
            ...
            if allow_snmp {
                if let Some(mut manual_device) = prompt_manual_snmp_device(ip, assume_yes).await? {
                    // Renumerar device_id para que sea único cuando hay varias IPs manuales
                    // (manual_1, manual_2, manual_3, ...). Esto es importante porque
                    // después se exportan al instalador.
                    manual_device.device_id = format!("manual_{}", i + 1);
                    devices.push(manual_device);
                }
            } else {
                println!("[INFO] Se omite validación SNMP para {} porque no hay conectividad ICMP confirmada.", ip);
            }
        }
    }
...
async fn phase_b_network_health(
    ...
    // Phase 2.5: iterar sobre TODAS las IPs manuales (no solo una).
    for ip in manual_target_ips {
        if seen_targets.insert(ip.clone()) {
            if ping_host(ip) {
                println!("[OK] Ping manual a {} exitoso.", ip);
                reachable_targets.insert(ip.clone());
            } else if gateway_ok {
    ...
    if devices.is_empty() && manual_target_ips.is_empty() {
        println!("[INFO] Sin dispositivos configurados ni IP manual. Fase B solo evaluará gateway y salida HTTPS.");
    } else if !manual_target_ips.is_empty() {
        println!("[INFO] {} IP(s) manual(es) a validar: {}", manual_target_ips.len(),
            manual_target_ips.iter().cloned().collect::<Vec<_>>().join(", "));
    }
...
fn prompt_manual_target_ip(assume_yes: bool) -> Result<Option<String>> {
    // Backward-compat: delega a la versión multi-IP y retorna la primera.
    let mut ips = prompt_manual_target_ips(assume_yes)?;
    Ok(ips.pop())
}

/// Phase 2.5: soporte multi-IP manual.
///
/// En lugar de pedir UNA sola IP y validar solo esa, ahora preguntamos al
/// usuario cuántas IPs desea validar y capturamos todas en orden. Cada una
/// se valida individualmente en Fase B (ping) y Fase C (SNMP).
///
/// El instalador (install_relay.sh) puede consumir TODAS las IPs validadas
/// del archivo de autocompletado y preguntar después si quiere agregar más.
fn prompt_manual_target_ips(assume_yes: bool) -> Result<Vec<String>> {
    if assume_yes || !io::stdin().is_terminal() {
        return Ok(Vec::new());
    }

    println!("[Asistente] No hay connection.config. Puede ejecutar diagnóstico ingresando una o más IPs manuales.");
    println!("[Asistente] Cada IP será validada con PING y SNMP para confirmar alcance antes de instalar.");
    println!();
    ...
    loop {
        if ips.is_empty() {
            let ip = read_input("Ingrese IP/host objetivo (o vacío para terminar): ")?;
            if ip.is_empty() {
                return Ok(Vec::new());
            }
            ips.push(ip);
        } else {
            // Para la segunda IP en adelante, ofrecer seguir
            let wants_more = ask_yes_no(&format!(
                "¿Desea validar OTRA IP además de {}? (y/N): ",
                ips.join(", ")
            ))?;
            if !wants_more {
                return Ok(ips);
            }
            let ip = read_input("Ingrese la siguiente IP/host: ")?;
            if ip.is_empty() {
                return Ok(ips);
            }
            ips.push(ip);
...
async fn prompt_manual_snmp_device(ip: &str, assume_yes: bool) -> Result<Option<DeviceConfig>> {
    ...
    // Phase 2.5: Mostrar la IP explícitamente para que el operador sepa
    // a cuál dispositivo corresponde este prompt (importante cuando se
    // validan múltiples IPs consecutivas).
    let wants_snmp = ask_yes_no(&format!(
        "¿Desea ejecutar prueba SNMP manual para {}? (Y/n): ",
        ip
    ))?;
    if !wants_snmp {
        return Ok(None);
    ...
    let mut device = DeviceConfig {
        ...
        // SSH audit fields — disabled by default in Smart Tester;
        // users can enable audit interactively in a future enhancement.
        ssh_enabled: false,
        ssh_host: None,
        ssh_port: None,
        ssh_username: None,
    ...
    match version_choice.as_str() {
        ...
        "3" => {
            ...
            if device.v3_auth_protocol != "NONE" {
                ...
                println!("[INFO] La contraseña SNMPv3 debe tener al menos 8 caracteres y será confirmada.");
                // Phase 2.10: confirmación de contraseña (doble input)
                let auth_password = read_password_with_confirm(
                    "Auth password SNMPv3 (mín. 8 caracteres): ",
                    show_auth_pass,
                )?;
                // Validar longitud mínima SNMPv3
                if auth_password.len() < 8 {
                    println!("[WARN] SNMPv3 típicamente requiere contraseñas de al menos 8 caracteres.");
                }
                ...
                if device.v3_priv_protocol != "NONE" {
                    ...
                    println!("[INFO] La contraseña de privacidad SNMPv3 será confirmada.");
                    // Phase 2.10: confirmación de contraseña (doble input)
                    let priv_password = read_password_with_confirm(
                        "Priv password SNMPv3 (mín. 8 caracteres): ",
                        show_priv_pass,
                    )?;
...
fn read_password(prompt: &str) -> Result<String> {
    ...
    if echo_disabled {
        ...
        println!(); // Nueva línea ya que Enter no se mostrará
    }
...
/// Lee una contraseña y luego pide confirmación. Si no coinciden, vuelve
/// a pedir hasta 3 intentos. Phase 2.10.
fn read_password_with_confirm(prompt: &str, show: bool) -> Result<String> {
    let max_attempts = 3;
    let mut attempt = 1;
    loop {
        // Primer input
        let pwd1 = if show {
            read_required_input(prompt)?
        } else {
            read_password(prompt)?
        };

        // Segundo input (confirmación)
        let confirm_prompt = format!("Confirmar {}: ",
            prompt.trim_end_matches(':').trim());
        let pwd2 = if show {
            read_required_input(&confirm_prompt)?
        } else {
            read_password(&confirm_prompt)?
        };
        ...
        println!(
            "[WARN] Las contraseñas no coinciden. Intento {} de {}.",
            attempt, max_attempts
        );
        attempt += 1;
        if attempt > max_attempts {
            anyhow::bail!("Demasiados intentos fallidos de confirmación de contraseña.");
        }
    }
}
...
///
/// Phase 2.5: el archivo ahora usa formato multi-dispositivo con secciones
/// [device_N] para soportar múltiples IPs validadas por el Smart Tester.
/// Formato:
///
///   # NESS Smart Tester — Datos de autocompletado
///   [device_1]
///   ip=192.168.10.17
///   port=161
///   snmp_version=3
...
fn export_autocomplete_data(devices: &[DeviceConfig], validated_ids: &HashSet<String>) {
    ...
    content.push_str("# Phase 2.5: formato multi-device con secciones [device_N]\n");

    for (i, device) in manual_validated.iter().enumerate() {
