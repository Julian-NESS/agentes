        ...
        if [[ "$snmp_version_choice" == "1" ]]; then
            ...
            if [[ "$v3_auth_protocol" != "NONE" ]]; then
                while true; do
                    ...
                    echo ""
                    if [[ ${#v3_auth_password} -ge 8 ]]; then break; else echo -e "${RED}  ❌ Error: La contraseña debe tener al menos 8 caracteres${NC}"; fi
                done
            fi
            ...
            if [[ "$v3_priv_protocol" != "NONE" ]]; then
                while true; do
                    ...
                    echo ""
                    if [[ ${#v3_priv_password} -ge 8 ]]; then break; else echo -e "${RED}  ❌ Error: La contraseña debe tener al menos 8 caracteres${NC}"; fi
                done
            fi
        fi
...
if [[ "$SILENT_MODE" != "true" && -f "$AUTOCOMPLETE_FILE" ]]; then
    ...
    if [[ "$USE_AUTOCOMPLETE" =~ ^[Yy]$ ]] || [[ -z "$USE_AUTOCOMPLETE" ]]; then
        ...
        # --- Paso 3: Auto-detección inteligente (sin selección manual) ---
        # El agente detectará automáticamente el vendor y tipo de dispositivo
        # vía sysObjectID y sysDescr durante la recolección SNMP.
        AC_VENDOR="generic"
        log_message "SUCCESS" "Dispositivo configurado para auto-detección: $AC_IP"
